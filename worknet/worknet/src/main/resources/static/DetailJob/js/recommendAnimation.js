 const testBoxValue = document.getElementsByClassName('testBox');
let testBoxArrayValue = Array.from(testBoxValue);
let x = 1;

function shift(){
    const firstTemp = testBoxArrayValue.shift();
    return firstTemp;
}
function push(first){  
    testBoxArrayValue.push(first);
}
function anime() {
    ...
    for(let i=2; i>=0; i--) {
        TweenLite.to(testBoxArrayValue[i], 0.3,
            {top: marginTemp,});
        marginTemp = marginTemp - marginMinus;
    }
    TweenLite.to(testBoxArrayValue[9], 0.3,
        {top: "120%"
    });
    ...
    // 실행 후 값 거꾸로 다시 삽입시킴
        ...
        let z = 1;
        testBoxArrayValue.some((value,index) => {
            let first = shift();
            push(first);
            console.log(value);
            z++;
...
function clickEvent() {
    const startButton = document.getElementById('reButton');
