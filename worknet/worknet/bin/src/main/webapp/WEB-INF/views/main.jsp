...
<body>
    <div class="main">
        ...
        <div class="nav"> 
          ...
          <a href="/jobs"><span>Home</span></a>
          <a href="/jobs/category.do"><span>Search</span></a>
...
<c:if test="${not empty recSet.recommendList}">    
	<div class="sliderLayoutContainer">
		...
		<c:forEach var="rec" items="${recSet.recommendList}" varStatus="status">
			  <article class="comandSlider__item">
			    <header>
			      <p class="comandSlider__item_name"><a style="color:white;" href="/jobs/detailJob?wantedAuthNo=${rec.wantedAuthNo}&IsRecommend=${recSet.IsRecommend}">${rec.corpNm}</a></p>
			      <p class="comandSlider__item_status">${rec.wantedTitle}</p>
			    ...
			    <div class="comandSlider__item_contact"><span>학력</span>${rec.eduNm}</div>
    ...
    <h3 class="test"> 추천 분야에 대한 검색결과를 제공 받으세요! </h3>
    <div class="recommandContainer_out_Layout">
    ...
    <span class="mainContainerNewTitle"> 신규 등록 공고 </span> <span class="mainContainerRecommendTitle"> 추천 등록 공고 </span>
	
	<!--  두줄 명세 테스트 2020-09-23 -->
	<c:forEach var="jobList" items="${jobs}" varStatus="status">
		<div class="twoRowcontentContainer">
			<div class="newContentContainer">
				<div class="companyName">
					<a href="/jobs/detailJob?wantedAuthNo=${jobList.wantedAuthNo}"><span>${jobList.corpNm}</span></a>
				</div>
			...
			<c:if test="${not empty recSet.recommendList}">
				<div class="recommendContentContainer">
					<div class="companyName">
						<a href="/jobs/detailJob?wantedAuthNo=${recSet.recommendList[status.index].wantedAuthNo}&IsRecommend=${recSet.IsRecommend}"><span>${recSet.recommendList[status.index].corpNm}</span></a>
					</div>
					<div class="companyDetail">${recSet.recommendList[status.index].wantedTitle}</div>
					<div class="companyDetail">${recSet.recommendList[status.index].salTpNm}</div>
					<div class="companyUrl">${recSet.recommendList[status.index].workRegion}</div>
					<div class="classification">
						<div class="recruitmentForm">${recSet.recommendList[status.index].receiptCloseDt}</div>
					</div>
				</div>
			</c:if>
		</div>
